...
def splat_rgb_img(ret, ratio, R_w2t, t_w2t, j, H, W, focal, fwd_flow):
    ...
    pts_mid_e_local = se3_transform_points(
        pts_mid_e_G, R_w2t.unsqueeze(0).unsqueeze(0), t_w2t.unsqueeze(0).unsqueeze(0)
    )
    ...
    splat_raw_rgba_dy = softsplat.FunctionSoftsplat(
        tenInput=raw_rgba, tenFlow=flow_2d, tenMetric=None, strType="average"
    )

    # splatting for static nerf
    pts_rig_e_local = se3_transform_points(
        pts_ref_e_G, R_w2t.unsqueeze(0).unsqueeze(0), t_w2t.unsqueeze(0).unsqueeze(0)
    )
    ...
    raw_rgba_rig = (
        raw_rgba_rig[:, :, j, :].permute(2, 0, 1).unsqueeze(0).contiguous().cuda()
    )

    splat_raw_rgba_rig = softsplat.FunctionSoftsplat(
        tenInput=raw_rgba_rig, tenFlow=flow_2d_rig, tenMetric=None, strType="average"
    )
...
def render_slowmo_bt(
    ...
    for i, cur_time in enumerate(
        np.linspace(target_idx - 10.0, target_idx + 10.0, 200 + 1).tolist()
    ):
        ...
        int_rot, int_trans = linear_pose_interp(
        ...
        int_poses = np.concatenate(
            [int_poses[:3, :4], np.array([0.0, 0.0, 0.0, 1.0])[np.newaxis, :]], axis=0
        )
        ...
        img_idx_embed_1 = (np.floor(cur_time)) / float(num_img) * 2.0 - 1.0
        img_idx_embed_2 = (np.floor(cur_time) + 1) / float(num_img) * 2.0 - 1.0
        ...
        for j in range(0, num_sample):
            (
                ...
                splat_alpha_rig_1,
                splat_rgb_rig_1,
            ) = splat_rgb_img(ret1, ratio, R_w2t, t_w2t, j, H, W, focal, True)
            (
                splat_alpha_dy_2,
                splat_rgb_dy_2,
                splat_alpha_rig_2,
                splat_rgb_rig_2,
            ) = splat_rgb_img(ret2, 1.0 - ratio, R_w2t, t_w2t, j, H, W, focal, False)

            final_rgb += (
                T_i
                * (
                    splat_alpha_dy_1 * splat_rgb_dy_1
                    + splat_alpha_rig_1 * splat_rgb_rig_1
                )
                * (1.0 - ratio)
            )
            final_rgb += (
                ...
                * (
                    ...
                    + splat_alpha_rig_2 * splat_rgb_rig_2
                )
                * ratio
            )
            ...
            alpha_1_final = (
                1.0 - (1.0 - splat_alpha_dy_1) * (1.0 - splat_alpha_rig_1)
            ) * (1.0 - ratio)
            alpha_2_fianl = (
                1.0 - (1.0 - splat_alpha_dy_2) * (1.0 - splat_alpha_rig_2)
            ) * ratio
...
def render_lockcam_slowmo(
    ...
    for i, cur_time in enumerate(
        np.linspace(target_idx - 8.0, target_idx + 8.0, 160 + 1).tolist()
    ):
        ratio = cur_time - np.floor(cur_time)
...
def render_sm(
    ...
    **kwargs,
    ...
    # Render and reshape
    all_ret = batchify_rays_sm(
        img_idx, chain_bwd, chain_5frames, num_img, rays, chunk, **kwargs
    )
    for k in all_ret:
...
def batchify_rays_sm(
    img_idx, chain_bwd, chain_5frames, num_img, rays_flat, chunk=1024 * 16, **kwargs
):
...
def render_rays_sm(
    ...
    rgb_map_rig, depth_map_rig, raw_rgba_rigid, raw_blend_w = get_rigid_outputs(
        pts_ref,
        viewdirs,
        rigid_network_query_fn,
        network_rigid,
    ...
    raw_rgb, raw_alpha = raw2rgba_blend_slowmo(
        raw_rgba_ref, raw_blend_w, z_vals, rays_d, raw_noise_std
    )
    raw_rgb_rigid, raw_alpha_rigid = raw2rgba_blend_slowmo(
        raw_rgba_rigid, (1.0 - raw_blend_w), z_vals, rays_d, raw_noise_std
    )

    ret = {
        "raw_rgb": raw_rgb,
        "raw_alpha": raw_alpha,
        "raw_rgb_rigid": raw_rgb_rigid,
        "raw_alpha_rigid": raw_alpha_rigid,
...
def render_bullet_time(
    ...
    for i in range(0, (render_poses.shape[0])):
        ...
        depth = torch.clamp(
            ret["depth_map_ref"] / percentile(ret["depth_map_ref"], 97), 0.0, 1.0
        )  # 1./disp
        rgb = (
            ret["rgb_map_ref"].cpu().numpy()
        )  # .append(ret['rgb_map_ref'].cpu().numpy())
...
def create_nerf(args):
    ...
    model_rigid = Rigid_NeRF(
        D=args.netdepth,
        W=args.netwidth,
        input_ch=input_rigid_ch,
        output_ch=output_ch,
        skips=skips,
        input_ch_views=input_ch_views,
        use_viewdirs=args.use_viewdirs,
    ...
    rigid_network_query_fn = lambda inputs, viewdirs, network_fn: run_network(
        inputs,
        viewdirs,
        network_fn,
...
def raw2outputs_blending(raw_dy, raw_rigid, raw_blend_w, z_vals, rays_d, raw_noise_std):
    act_fn = F.relu
    ...
    Ts = torch.cumprod(
        torch.cat(
            [
                ...
                (1.0 - alpha_dy) * (1.0 - alpha_rig) + 1e-10,
            ],
            -1,
        ),
        -1,
    ...
    # union map
    rgb_map = torch.sum(
        weights_dy[..., None] * rgb_dy + weights_rig[..., None] * rgb_rigid, -2
    )
    ...
    alpha_dynamic = 1.0 - torch.exp(-opacity_dy * dists)
    weights_dynamic = (
        alpha_dynamic
        * torch.cumprod(
            torch.cat(
                [torch.ones((alpha_dynamic.shape[0], 1)), 1.0 - alpha_dynamic + 1e-10],
                -1,
            ),
            -1,
        )[:, :-1]
    ...
    return rgb_map, depth_map, rgb_map_dy, depth_map_dynamic, weights_dynamic
...
def raw2outputs_warp(raw_p, z_vals, rays_d, raw_noise_std=0):
    ...
    alpha = 1.0 - torch.exp(-opacity * dists)
...
def get_rigid_outputs(
    ...
    rgb_map_rig, weights_rig, depth_map_rig = raw2outputs(
        raw_rgba_rigid, z_vals, rays_d, raw_noise_std, white_bkgd=False
    )
...
def render_rays(
    ...
    (
        ...
        depth_map_ref_dy,
        weights_ref_dy,
    ) = raw2outputs_blending(
        raw_rgba_ref, raw_rgba_rigid, raw_blend_w, z_vals, rays_d, raw_noise_std
    )
    ...
    # render from t - 1
    rgb_map_prev_dy, _, weights_prev_dy = raw2outputs_warp(
        raw_rgba_prev, z_vals, rays_d, raw_noise_std
    )
    ...
    rgb_map_post_dy, _, weights_post_dy = raw2outputs_warp(
        raw_rgba_post, z_vals, rays_d, raw_noise_std
    )
    ...
    if chain_bwd:
        ...
        img_idx_rep_prevprev = torch.ones_like(pts[:, :, 0:1]) * (
            img_idx - 2.0 / num_img * 2.0
        )
        pts_prevprev = torch.cat(
            [(pts_prev[:, :, :3] + raw_sf_prev2prevprev), img_idx_rep_prevprev], -1
        )
        ...
        if chain_5frames:
            ...
            # render from t - 2
            rgb_map_prevprev_dy, _, weights_prevprev_dy = raw2outputs_warp(
            ...
            ret["prob_map_pp"] = compute_2d_prob(
                weights_prevprev_dy, raw_prob_ref2prevprev
            )
        ...
        # render points at t + 2
        img_idx_rep_postpost = torch.ones_like(pts[:, :, 0:1]) * (
            img_idx + 2.0 / num_img * 2.0
        )
        pts_postpost = torch.cat(
            [(pts_post[:, :, :3] + raw_sf_post2postpost), img_idx_rep_postpost], -1
        )
        ...
        if chain_5frames:
            ...
            # render from t + 2
            rgb_map_postpost_dy, _, weights_postpost_dy = raw2outputs_warp(
                raw_rgba_postpost, z_vals, rays_d, raw_noise_std
            )

            raw_prob_post2postpost = raw_post[:, :, 11]
            raw_prob_ref2postpost = 1.0 - (1.0 - raw_prob_ref2post) * (
                1.0 - raw_prob_post2postpost
            )

            ret["prob_map_pp"] = compute_2d_prob(
                weights_postpost_dy, raw_prob_ref2postpost
            )
