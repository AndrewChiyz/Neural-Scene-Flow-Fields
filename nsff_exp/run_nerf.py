...
def config_parser():
    ...
    parser.add_argument("--render_bt", action="store_true", help="render bullet time")
    ...
    parser.add_argument("--target_idx", type=int, default=10, help="target_idx")
    parser.add_argument(
        "--num_extra_sample", type=int, default=512, help="num_extra_sample"
    )
    parser.add_argument(
        "--decay_depth_w", action="store_true", help="decay depth weights"
    )
    parser.add_argument(
        "--use_motion_mask",
        action="store_true",
    ...
    parser.add_argument(
        "--decay_optical_flow_w", action="store_true", help="decay optical flow weights"
    )

    parser.add_argument(
        "--w_depth", type=float, default=0.04, help="weights of depth loss"
    )
    parser.add_argument(
        "--w_optical_flow",
        type=float,
    ...
    parser.add_argument(
        "--w_sm", type=float, default=0.1, help="weights of scene flow smoothness"
    )
    parser.add_argument(
        "--w_sf_reg",
        type=float,
    ...
    parser.add_argument(
        "--w_cycle", type=float, default=0.1, help="weights of cycle consistency"
    )
    parser.add_argument(
        "--w_prob_reg", type=float, default=0.1, help="weights of disocculusion weights"
    )
    parser.add_argument(
        "--decay_iteration",
        type=int,
...
def train():
    ...
    if args.render_bt:
        ...
        img_idx_embed = target_idx / float(num_img) * 2.0 - 1.0
    ...
    if args.render_slowmo_bt:
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(
                basedir,
                expname,
                "render-slowmo_bt_{}_{:06d}".format(
    ...
    for i in range(start, N_iters):
        ...
        if img_i == 0:
            flow_fwd, fwd_mask = read_optical_flow(
                args.datadir, img_i, args.start_frame, fwd=True
            )
            flow_bwd, bwd_mask = np.zeros_like(flow_fwd), np.zeros_like(fwd_mask)
        elif img_i == num_img - 1:
            flow_bwd, bwd_mask = read_optical_flow(
                args.datadir, img_i, args.start_frame, fwd=False
            )
        ...
        if TEST:
            ...
            plt.subplot(2, 3, 2)
            plt.imshow(flow_to_image(flow_fwd) / 255.0 * fwd_mask[..., np.newaxis])

            plt.subplot(2, 3, 3)
            plt.imshow(flow_to_image(flow_bwd) / 255.0 * bwd_mask[..., np.newaxis])
        ...
        if N_rand is not None:
            ...
            if args.use_motion_mask and i < decay_iteration * 1000:
                ...
                select_inds_all = np.random.choice(
            ...
            target_mask = mask_gt[select_coords[:, 0], select_coords[:, 1]].unsqueeze(
                -1
            )

            target_of_fwd = flow_fwd[select_coords[:, 0], select_coords[:, 1]]
            target_fwd_mask = fwd_mask[
            ...
            target_bwd_mask = bwd_mask[
                select_coords[:, 0], select_coords[:, 1]
            ].unsqueeze(
                -1
            )  # .repeat(1, 2)

        img_idx_embed = img_i / num_img * 2.0 - 1.0
        ...
        # get the index of the posterior frame index, check the boundary according to the length of frame
        ...
        weight_map_post = ret["prob_map_post"]  # [N_rays, 1]
        weight_map_prev = ret["prob_map_prev"]  # [N_rays, 1]

        weight_post = 1.0 - ret["raw_prob_ref2post"]  # [N_rays, N_samples]
        weight_prev = 1.0 - ret["raw_prob_ref2prev"]  # [N_rays, N_samples]
        # second term in equation (8)
        prob_reg_loss = args.w_prob_reg * (
            torch.mean(torch.abs(ret["raw_prob_ref2prev"]))
        ...
        render_loss += compute_mse(
            ret["rgb_map_post_dy"], target_rgb, weight_map_post.unsqueeze(-1)
        )
        render_loss += compute_mse(
            ret["rgb_map_prev_dy"], target_rgb, weight_map_prev.unsqueeze(-1)
        )

        # union rendering loss
        render_loss += img2mse(
            ret["rgb_map_ref"][:N_rand, ...], target_rgb[:N_rand, ...]
        )
        # Equation (9) cycle consistency loss
        sf_cycle_loss = args.w_cycle * compute_mae(
            ret["raw_sf_ref2post"],
            -ret["raw_sf_post2ref"],
        ...
        # regularization loss : encourage the motion between a reference time step t to t+1 should be small
        sf_reg_loss = args.w_sf_reg * (
            torch.mean(torch.abs(ret["raw_sf_ref2prev"]))
        ...
        # dynamic depth component loss
        depth_loss = w_depth * compute_depth_loss(
            ret["depth_map_ref_dy"], -target_depth
        )
        ...
        sf_sm_loss += args.w_sm * compute_sf_lke_loss(
            ret["raw_pts_ref"], ret["raw_pts_post"], ret["raw_pts_prev"], H, W, focal
        )
        sf_sm_loss += args.w_sm * compute_sf_lke_loss(
            ret["raw_pts_ref"], ret["raw_pts_post"], ret["raw_pts_prev"], H, W, focal
        )
        ...
        if chain_bwd:
            sf_sm_loss += args.w_sm * compute_sf_sm_loss(
                ret["raw_pts_prev"], ret["raw_pts_pp"], H, W, focal
            )
            sf_sm_loss += args.w_sm * compute_sf_lke_loss(
            ...
            sf_sm_loss += args.w_sm * compute_sf_sm_loss(
                ret["raw_pts_post"], ret["raw_pts_pp"], H, W, focal
            )
        ...
        if chain_5frames:
            ...
            raw_weight_p2pp = 1.0 - ret["raw_prob_p2pp"]
            prob_reg_loss += args.w_prob_reg * torch.mean(
                torch.abs(ret["raw_prob_p2pp"])
            )

            render_loss += compute_mse(
                ret["rgb_map_pp_dy"], target_rgb, weight_map_pp.unsqueeze(-1)
            )
                ...
                torch.save(
                    {
                        ...
                        "network_rigid": render_kwargs_train[
                            "network_rigid"
                        ].state_dict(),
        ...
        if i % args.i_img == 0 and i > 0:
            ...
            flow_fwd_rgb = torch.Tensor(flow_to_image(flow_fwd) / 255.0)  # .cuda()
            writer.add_image(
                "val/gt_flow_fwd", flow_fwd_rgb, global_step=i, dataformats="HWC"
            )
            flow_bwd_rgb = torch.Tensor(flow_to_image(flow_bwd) / 255.0)  # .cuda()
            writer.add_image(
                "val/gt_flow_bwd", flow_bwd_rgb, global_step=i, dataformats="HWC"
            )
            ...
            with torch.no_grad():
                ...
                pose_prev = poses[max(img_i - 1, 0), :3, :4]
                render_of_fwd, render_of_bwd = compute_optical_flow(
                    pose_post, pose, pose_prev, H, W, focal, ret, n_dim=2
                )

                render_flow_fwd_rgb = torch.Tensor(
                    flow_to_image(render_of_fwd.cpu().numpy()) / 255.0
                )  # .cuda()
                render_flow_bwd_rgb = torch.Tensor(
                    flow_to_image(render_of_bwd.cpu().numpy()) / 255.0
                )  # .cuda()

                writer.add_image(
                    "val/rgb_map_ref",
                    torch.clamp(ret["rgb_map_ref"], 0.0, 1.0),
                    global_step=i,
                    dataformats="HWC",
                )
                writer.add_image(
                ...
                writer.add_image(
                    "val/monocular_disp",
                    torch.clamp(target_depth / percentile(target_depth, 97), 0.0, 1.0),
                    global_step=i,
                    dataformats="HW",
                )

                writer.add_image(
                    "val/render_flow_fwd_rgb",
                    render_flow_fwd_rgb,
                    global_step=i,
                    dataformats="HWC",
                )
                writer.add_image(
                    "val/render_flow_bwd_rgb",
                    render_flow_bwd_rgb,
                    global_step=i,
                    dataformats="HWC",
                )
