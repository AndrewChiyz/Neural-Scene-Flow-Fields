...
def _load_data(
    ...
    # get image files path with start and end frame indices
    # pose.shape with [3, 5, 75(or number of frames)]
...
def read_MiDaS_disp(disp_fi, disp_rescale=10.0, h=None, w=None):
...
def load_nvidia_data(
    ...
    poses, bds, imgs = _load_data(
        basedir, start_frame, end_frame, height=final_height, evaluation=True
    )
    ...
    close_disp, inf_disp = bds.min() * 0.9, bds.max() * 5.0
    dt = 0.75
    mean_dz = 1.0 / (((1.0 - dt) / close_disp + dt / inf_disp))
    focal = mean_dz

    # Get radii for spiral path
    shrink_factor = 0.8
    zdelta = close_disp * 0.1
...
def load_llff_data(
    ...
    poses, bds, imgs, disp, masks, motion_coords = _load_data(
        basedir, start_frame, end_frame, height=final_height, evaluation=False
    )
...
def create_bt_poses(hwf):
    ...
    for i in range(num_frames):
        ...
        y_trans = (
            max_trans * np.cos(2.0 * np.pi * float(i) / float(num_frames)) / 2.0
        )  # * 3.0 / 4.0
        z_trans = 0.0  # z_shift + max_trans * np.sin(2.0 * np.pi * float(init_pos + i) / float(num_frames))
