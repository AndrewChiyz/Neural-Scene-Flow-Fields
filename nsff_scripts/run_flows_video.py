...
def motion_segmentation(
    basedir, threshold, input_semantic_w=1024, input_semantic_h=576
):
    ...
    K = np.eye(3)
    K[0, 0] = hwf[2] * scale_x
    K[0, 2] = hwf[1] / 2.0 * scale_x
    K[1, 1] = hwf[2] * scale_y
    K[1, 2] = hwf[0] / 2.0 * scale_y
    ...
    for i in range(0, num_frames):  # len(perm) - 1):
        ...
        p_post_h = np.concatenate(
            (p_post_h, np.ones((p_post_h.shape[0], 1))), axis=-1
        ).T
        ...
        p_prev_h = np.concatenate(
            (p_prev_h, np.ones((p_prev_h.shape[0], 1))), axis=-1
        ).T
        ...
        cv2.imwrite(
            os.path.join(save_mask_dir, im_ref.name.replace(".jpg", ".png")),
            np.uint8(255 * (0.0 + motion_mask)),
        )

    # RUN SEMANTIC SEGMENTATION
    img_dir = os.path.join(basedir, "images")
    img_path_list = sorted(glob.glob(os.path.join(img_dir, "*.jpg"))) + sorted(
    ...
    for i in range(0, len(img_path_list)):
        ...
        semantic_mask = run_maskrcnn(
            netMaskrcnn, img_path, input_semantic_w, input_semantic_h
        )
        cv2.imwrite(
            os.path.join(semantic_mask_dir, img_name.replace(".jpg", ".png")),
            semantic_mask,
        )
    ...
    for mask_path in mask_path_list:
        ...
        cv2.imwrite(
            os.path.join(save_mask_dir, "%s" % mask_path.split("/")[-1]),
...
def load_image(imfile, input_flow_w):
    ...
    img = cv2.resize(
        ...
        (
            input_flow_w,
            int(round(float(img.shape[0]) / float(img.shape[1]) * input_flow_w)),
        ),
        cv2.INTER_LINEAR,
    )
...
def read_img(img_dir, img1_name, img2_name):
    return (
        cv2.imread(os.path.join(img_dir, img1_name + ".png")),
...
def run_optical_flows(args):
    ...
    with torch.no_grad():
        images = glob.glob(os.path.join(basedir, "images/", "*.png")) + glob.glob(
            os.path.join(basedir, "images/", "*.jpg")
        )
        ...
        for i in range(images.shape[0] - 1):
            ...
            img1 = cv2.resize(
                np.uint8(np.clip(image1[0].cpu().numpy().transpose(1, 2, 0), 0, 255)),
                (img_train.shape[1], img_train.shape[0]),
                cv2.INTER_LINEAR,
            )
            img2 = cv2.resize(
                np.uint8(np.clip(image2[0].cpu().numpy().transpose(1, 2, 0), 0, 255)),
                (img_train.shape[1], img_train.shape[0]),
                cv2.INTER_LINEAR,
            )
            ...
            if VIZ:
                ...
                plt.subplot(2, 3, 2)
                plt.imshow(flow_viz.flow_to_image(fwd_flow) / 255.0)
                plt.subplot(2, 3, 3)
                plt.imshow(flow_viz.flow_to_image(bwd_flow) / 255.0)

                plt.subplot(2, 3, 5)
                plt.imshow(
                    flow_viz.flow_to_image(fwd_flow)
                    / 255.0
                ...
                plt.subplot(2, 3, 6)
                plt.imshow(
                    flow_viz.flow_to_image(bwd_flow)
                    / 255.0
                ...
                cv2.imwrite(
                    "./viz_warp_imgs/im_%05d_fwd.jpg" % (i), warped_im2[..., ::-1]
                )
                cv2.imwrite(
                    "./viz_warp_imgs/im_%05d_bwd.jpg" % (i + 1), warped_im0[..., ::-1]
                )

            np.savez(
                os.path.join(of_dir, "%05d_fwd.npz" % i), flow=fwd_flow, mask=fwd_mask
            )
            np.savez(
                os.path.join(of_dir, "%05d_bwd.npz" % (i + 1)),
                flow=bwd_flow,
                mask=bwd_mask,
            )
...
if __name__ == "__main__":
    ...
    motion_segmentation(
        args.data_path, args.epi_threshold, args.input_semantic_w, args.input_semantic_h
    )
